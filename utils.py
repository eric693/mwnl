...
class Normlize_tx:
    ...
    def apply(self, _input):
        ...
        _norm = _dim**0.1/ torch.sqrt(torch.sum(_input**2, dim=1)) #0.1
...
#         _dim = _input.shape[1]//2 if self._iscomplex else _input.shape[1]
#         _norm = _dim**0.1 / torch.sqrt(torch.sum(_input**2, dim=1)) #0.1
#         _output = _input * _norm.view(-1, 1)
#         return F.relu(_output)  # 使用ReLU確保所有數字都是正值
...
class Channel:
    ...
    def fading(self, _input, _snr):
        # ref from DeepJSCC-f https://github.com/kurka/deepJSCC-feedbac
...
def copyStage1ckpts(frompath, topath, strs='resume_from_ce_'):
    ...
    files = glob.glob(os.path.join(frompath, '*99.pth')) #87
    for file in files:
