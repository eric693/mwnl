...
def quantize_and_convert_to_binary_with_integer_part(input_tensor, error_boundary):
    ...
    binary_tensor = torch.zeros(input_tensor.shape[0], input_tensor.shape[1], 16, dtype=torch.int)
        ...
        for j in range(quantized_tensor.shape[1]):
            ...
            if value < 0:
                ...
                if (value == -0.125) or(value == -0.375) or (value == -0.625) or (value == -0.875):  # Quantized value for -0.9252
                    binary_tensor[i, j, 5:10] = 0
                else:
                    binary_tensor[i, j, 5:10] = 1
            else:
                binary_tensor[i, j, :5] = 0  # Sign bits for positive values
                ...
                if (value == 1.125) or (value == 1.375) or (value == 1.625) or (value == 1.875):
                    binary_tensor[i, j, 5:10] = 1  # Integer part 1
                else:
                    binary_tensor[i, j, 5:10] = 0  # Integer part 0
            ...
            if (value >= 0 and value < 0.25) or (value >= 1 and value < 1.25):
                binary_tensor[i, j, 10:13] = 0  # Set 7th to 9th bits to 0, 0, 0
                binary_tensor[i, j, 13:16] = 0
            elif (value >= 0.25 and value < 0.5) or (value >= 1.25 and value < 1.5):
                binary_tensor[i, j, 10:13] = 1  # Set 7th to 9th bits to 1, 1, 1
                binary_tensor[i, j, 13:16] = 0
            elif (value >= 0.5 and value < 0.75) or (value >= 1.5 and value < 1.75):
                binary_tensor[i, j, 10:13] = 0  # Set 7th to 9th bits to 0, 0, 0
                binary_tensor[i, j, 13:16] = 1
            else:
                binary_tensor[i, j, 10:13] = 1  # Set 7th to 9th bits to 1, 1, 1
                binary_tensor[i, j, 13:16] = 1
...
def convert_binary_to_float_final(binary_tensor, quantization_values):
    ...
    for i in range(binary_tensor.shape[0]):
        for j in range(binary_tensor.shape[1]):
            ...
            integer_bits = binary_tensor[i, j, 5:10]
            fractional_classification_bits = binary_tensor[i, j, 10:13]
            final_classification_bits = binary_tensor[i, j, 13:16]

            # Determine the sign: Negative if the first 3 bits are 1
