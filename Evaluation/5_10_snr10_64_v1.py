...
def fixed_point_to_float_sign_5_5_6(x_fixed, int_bits, frac_bits):
    ...
    x_fixed = x_fixed.view(-1, int_bits + frac_bits + 31)


    sign = torch.where(torch.sum(x_fixed[:, :31], dim=1) > 15, -1, 1)


    int_part = x_fixed[:, 31:31+int_bits]
    int_part = torch.where(torch.sum(int_part, dim=1) > 15, 1, 0)


    frac_part = x_fixed[:, 31+int_bits:31+int_bits+frac_bits]
...
def do_test(input_data, encoder, decoder, normlize_layer, channel, len_batch):
    ...
    with torch.no_grad():
        ...
        print("Channel noise :")
        flipped = flip_bits(pred1, n=40)
        print(flipped)


        pred8 = fixed_point_to_float_sign_5_5_6(flipped, int_bits=31, frac_bits=2)
