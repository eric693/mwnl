...
def quantize_tensor_custom(input_tensor, error_boundary):
    ...
    for q_range, q_value in zip(quantization_ranges, quantization_values):
        ...
        mask = (quantized_tensor > lower_bound) & (quantized_tensor <= upper_bound)
        quantized_tensor[mask] = q_value

    quantized_tensor = torch.clamp(quantized_tensor, min=-2, max=2)

    return quantized_tensor

def float_to_binary_tensor(values, min_value, max_value, num_bits):
    values_flattened = values.flatten()  
    binary_tensor = torch.zeros(values_flattened.shape[0], num_bits, dtype=torch.int)
    
    for i in range(values_flattened.shape[0]):
        value = values_flattened[i].item()  
        value = min(max(value, min_value), max_value)
        normalized_value = (value - min_value) / (max_value - min_value)
        integer_value = int(normalized_value * (2**num_bits - 1))
        binary_str = format(integer_value, f'0{num_bits}b')
        binary_list = [int(bit) for bit in binary_str]
        binary_tensor[i] = torch.tensor(binary_list, dtype=torch.int)
    

    return binary_tensor.view(*values.shape, num_bits)


def binary_tensor_to_float(binary_tensor, min_value, max_value):
    if binary_tensor.dim() != 3:
        raise ValueError("binary_tensor should be 3-dimensional")

    num_bits = binary_tensor.size(2)
    outer_dim = binary_tensor.size(0)
    inner_dim = binary_tensor.size(1)
    values = []

    for i in range(outer_dim):
        for j in range(inner_dim):
            binary_list = binary_tensor[i][j].tolist()
    ...
    return torch.tensor(values).view(outer_dim, inner_dim)
...
def do_test(input_data, encoder, decoder, normlize_layer, channel, len_batch):
    ...
    with torch.no_grad():
        ...
        pred1 = quantize_tensor_custom(output, error_boundary)
        print(pred1)
        ...
        quantized_tensor = float_to_binary_tensor(pred1, min_value, max_value, num_bits)

        print(f"Original Fixed Point:\n{quantized_tensor}")
        
        # Channel noise
        BER_target = 0.00001
        error_mask = torch.rand(quantized_tensor.shape) < BER_target
        received_pred1 = quantized_tensor ^ error_mask.long()
        num_errors = torch.sum(quantized_tensor != received_pred1)
        total_bits_accumulated += quantized_tensor.numel()
        ...
        pred8 = binary_tensor_to_float(received_pred1, min_value, max_value)
        print(pred8)
