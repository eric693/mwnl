...
def quantize_and_convert_to_binary_with_integer_part(input_tensor, error_boundary):
    ...
    binary_tensor = torch.zeros(input_tensor.shape[0], input_tensor.shape[1], 15, dtype=torch.int32)
    for i in range(quantized_tensor.shape[0]):
        for j in range(quantized_tensor.shape[1]):
            ...
            if (value == 0.125) or(value == 0.375) or (value == 0.625) or (value == 0.875):  # Quantized value for -0.9252
                binary_tensor[i, j, 0:5] = 0
            else:
                binary_tensor[i, j, 0:5] = 1

        
            if (value == 1.125) or (value == 1.375) or (value == 1.625) or (value == 1.875):
                binary_tensor[i, j, 0:5] = 1  # Integer part 1
            else:
                binary_tensor[i, j, 0:5] = 0  # Integer part 0
       
            value = abs(value)

            if (value >= 0 and value < 0.25) or (value >= 1 and value < 1.25):
                binary_tensor[i, j, 5:10] = 0  # Set 7th to 9th bits to 0, 0, 0
                binary_tensor[i, j, 10:15] = 0
            elif (value >= 0.25 and value < 0.5) or (value >= 1.25 and value < 1.5):
                binary_tensor[i, j, 5:10] = 1  # Set 7th to 9th bits to 1, 1, 1
                binary_tensor[i, j, 10:15] = 0
            elif (value >= 0.5 and value < 0.75) or (value >= 1.5 and value < 1.75):
                binary_tensor[i, j, 5:10] = 0  # Set 7th to 9th bits to 0, 0, 0
                binary_tensor[i, j, 10:15] = 1
            else:
                binary_tensor[i, j, 5:10] = 1  # Set 7th to 9th bits to 1, 1, 1
                binary_tensor[i, j, 10:15] = 1
...
def convert_binary_to_float_final(binary_tensor, quantization_values):
    ...
    for i in range(binary_tensor.shape[0]):
        for j in range(binary_tensor.shape[1]):
            ...
            integer_bits = binary_tensor[i, j, 0:5]
            fractional_classification_bits = binary_tensor[i, j, 5:10]
            final_classification_bits = binary_tensor[i, j, 10:15]

            # Determine the integer part: 1 if the 6th bit is 1
            integer_part = 1 if integer_bits.sum() > 2 else 0

            # Determine the fractional part based on the fractional classification bits
            zero_count_fractional = (fractional_classification_bits == 0).sum().item()
            one_count_fractional = (fractional_classification_bits == 1).sum().item()

            # Determine the final fractional part based on the final classification bits
            zero_count_final = (final_classification_bits == 0).sum().item()
            one_count_final = (final_classification_bits == 1).sum().item()

            if zero_count_fractional > one_count_fractional:
                fractional_part = .125 if zero_count_final > one_count_final else .625
            else:
                fractional_part = .375 if zero_count_final > one_count_final else .875

            # Combine to form the original value
            restored_value = (integer_part + fractional_part) 
...
def do_test(input_data, encoder, decoder, normlize_layer, channel, len_batch):
    ...
    total_blocks_with_error = 0
    total_blocks_accumulated = 0
    ...
    with torch.no_grad():
        ...
        # Channel noise
        # BLER target
        BLER_target = 0.7  # Target Block Error Rate

        # Simulate BLER
        received_data = pred1.clone()
        num_blocks = received_data.size(1)  # Assuming second dimension is the number of blocks

        for block in range(num_blocks):
            block_error = False  # Flag to check if error has occurred in block

            if torch.rand(1).item() < BLER_target:
                # Introduce error in one random bit of the block
                bit_to_flip = torch.randint(0, received_data.size(2), (1,))
                received_data[0, block, bit_to_flip] ^= 1  # Flip the bit

                block_error = True  # Mark block as erroneous

            # Update the total blocks accumulated
            total_blocks_accumulated += 1

            # Update block error count if error in block
            if block_error:
                total_blocks_with_error += 1

        # Printing results
        print(f"Received Fixed Point with BLER {BLER_target}:\n{received_data}")
        print("bit vector convert float_point :")

        quantization_values = [0.125, 0.375, 0.625, 0.875, 1.125, 1.375, 1.625, 1.875]
        pred8 = convert_binary_to_float_final(received_data, quantization_values)
        print(pred8)
        ...
        # Calculate BER
        calculated_BLER = total_blocks_with_error / total_blocks_accumulated if total_blocks_accumulated > 0 else 0

        # Print out results
        print(f"Calculated BLER: {calculated_BLER}")
...
SemanticRL_example = [          
                    ...
                    #    'compounds consist water joined together between specific ratios arrangements usrp',
                         'planets engage in an eternal graceful dance around sun rise'
                       #  'vrb directly mapped to prb',
                        #  'from ue to gnb'
                      ]
# input_str = "['compounds consist water joined together between specific ratios arrangements usrp' ]"
