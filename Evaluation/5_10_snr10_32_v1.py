...
def fixed_point_to_float_sign_5_5_6(x_fixed, int_bits, frac_bits):
    ...
    x_fixed = x_fixed.view(-1, int_bits + frac_bits + 15)


    sign = torch.where(torch.sum(x_fixed[:, :15], dim=1) > 7, -1, 1)


    int_part = x_fixed[:, 15:15+int_bits]
    int_part = torch.where(torch.sum(int_part, dim=1) > 7, 1, 0)


    frac_part = x_fixed[:, 15+int_bits:15+int_bits+frac_bits]
...
def do_test(input_data, encoder, decoder, normlize_layer, channel, len_batch):
    ...
    with torch.no_grad():
        ...
        pred8 = fixed_point_to_float_sign_5_5_6(flipped, int_bits=15, frac_bits=2)
