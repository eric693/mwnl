...
def quantize_and_convert_to_binary(input_tensor, error_boundary):
    quantization_ranges = [(-2.0, -1.751), (-1.75, -1.51), (-1.5, -1.251), (-1.25, -1.001), (-1.0, -0.751), (-0.75, -0.51), (-0.5, -0.251), (-0.25, -0.001), (0.0, 0.249), (0.25, 0.499), (0.5, 0.749), (0.75, 0.99), (1.0, 1.249), (1.25, 1.499), (1.5, 1.749), (1.75, 1.99)]
    quantization_values = [-1.875, -1.625, -1.375, -1.125, -0.875, -0.625, -0.375, -0.125, 0.125, 0.375, 0.625, 0.875, 1.125, 1.375, 1.625, 1.875]
    ...
    for i in range(quantized_tensor.shape[0]):
        for j in range(quantized_tensor.shape[1]):
            ...
            abs_value = abs(value)
            if abs_value in quantization_values:
                binary_index = quantization_values.index(abs_value)
        
                binary_index = binary_index % 8
                binary_str = format(binary_index, '03b') 
                for k, bit in enumerate(binary_str):
                    binary_tensor[i, j, 5 + k] = int(bit)

    return binary_tensor

def binary_to_float(binary_tensor, quantization_values):
    if binary_tensor.dim() != 3 or binary_tensor.size(2) != 8:
        raise ValueError("Input tensor should be 3-dimensional with size 8 in the last dimension.")

    output_tensor = torch.zeros(binary_tensor.shape[0], binary_tensor.shape[1])
    for i in range(binary_tensor.shape[0]):
        for j in range(binary_tensor.shape[1]):
            binary_number = binary_tensor[i][j]
            sign = -1 if binary_number[:5].sum() >= 3 else 1
            quantized_index = int(''.join(map(str, binary_number[5:].tolist())), 2)
    
            if 0 <= quantized_index < len(quantization_values):
                float_value = quantization_values[quantized_index]
            else:
                float_value = 0  
            output_tensor[i][j] = sign * float_value

    return output_tensor


def binary_to_quantized_updated(binary_tensor, quantization_values):
    # Create a tensor to store the quantized values
    quantized_tensor = torch.zeros(binary_tensor.shape[0], binary_tensor.shape[1])

    for i in range(binary_tensor.shape[0]):
        for j in range(binary_tensor.shape[1]):
            # Extract the sign (first 5 bits) and index (last 3 bits)
            sign_bits = binary_tensor[i, j, :5]
            index_bits = binary_tensor[i, j, 5:]

            # Determine if the value is positive or negative based on the sign bits
            # Positive if more than two zeros, otherwise negative
            is_negative = sign_bits.sum() > 2

            # Convert the binary index to an integer
            binary_index_str = ''.join([str(int(bit)) for bit in index_bits])
            index = int(binary_index_str, 2)

            # Map the index to the quantized value
            quantized_value = quantization_values[index]

            # Apply the sign
            if is_negative:
                quantized_value = -quantized_value

            quantized_tensor[i][j] = quantized_value

    return quantized_tensor

def binary_to_quantized(binary_tensor, quantization_values):
    # Create a tensor to store the quantized values
    quantized_tensor = torch.zeros(binary_tensor.shape[0], binary_tensor.shape[1])
...
embeds_shared = Embeds(vocab_size=300, num_hidden=256).to(device)
...
def do_test(input_data, encoder, decoder, normlize_layer, channel, len_batch):
    ...
    with torch.no_grad():
        ...
        pred1 = quantize_and_convert_to_binary(output, error_boundary)
        ...
        pred8 = binary_to_quantized_updated(received_pred1, quantization_values)
        print(pred8)
