...
def convert_binary_to_float(binary_tensor, quantization_values):
    ...
    for i in range(binary_tensor.shape[0]):
        for j in range(binary_tensor.shape[1]):
            ...
            integer_bits = binary_tensor[i, j, :3]
            fractional_bits = binary_tensor[i, j, 3:]
...
embeds_shared = Embeds(vocab_size=84, num_hidden=512).to(device)
encoder = LSTMEncoder(channel_dim=channel_dim, embedds=embeds_shared).to(device)
decoder = LSTMDecoder(channel_dim=channel_dim, embedds=embeds_shared, vocab_size = 84).to(device)
