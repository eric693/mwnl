...
def quantize_and_convert_to_binary_with_integer_part(input_tensor, error_boundary):
    ...
    quantization_ranges = [
                           (-2, -1.5), (-1.4999, -1.0), (-0.9999, -0.5), (-0.4999, -0.1),
                           (0.0, 0.4999), (0.5, 0.9999), (1.0, 1.4999), (1.5, 1.999),
                       ]
    quantization_values = [-1.75, -1.25, -0.75, -0.25, 
                           0.25, 0.75, 1.25, 1.75]
    # Quantize the tensor
    ...
    binary_tensor = torch.zeros(input_tensor.shape[0], input_tensor.shape[1], 2, dtype=torch.int32)
...
def convert_binary_to_float(binary_tensor, quantization_values):
    ...
    for i in range(binary_tensor.shape[0]):
        for j in range(binary_tensor.shape[1]):
            ...
            # fractional_value = fractional_mapping.get(tuple(fractional_bits.tolist()), None)
...
def do_test(input_data, encoder, decoder, normlize_layer, channel, len_batch):
    ...
    with torch.no_grad():
        ...
        quantization_values = [-1.75, -1.25, -0.75, -0.25, 0.25, 0.75, 1.25, 1.75]
