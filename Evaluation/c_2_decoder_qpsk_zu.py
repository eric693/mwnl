...
def decimal_to_bits(decimal):
    ...
    return torch.tensor([int(bit) for bit in format(decimal, '02b')])
...
def do_test(input_data, encoder, decoder, normlize_layer, channel, len_batch):
    ...
    with torch.no_grad():
        ...
        # replaced_tensor = torch.tensor([[[1, 1], [1, 1]]])
        print(f"Binarizer:\n{replaced_tensor}")
        bit_sequences = replaced_tensor.view(-1, 2)
        ...
        modulated_symbols = decimal_to_qpsk(decimal_numbers)
        print("Modulation:")
...
print("Message encoded. Encoded bits: 1")
print(processed_str)
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--ckpt_pathCE_Stage", type=str, default='./ckpt_AWGN_CE_Stage')
    ...
    with open('log.txt', 'w') as log_file:
        for input_str in SemanticRL_example:
            ...
            for ckpt_dir in [args.ckpt_pathCE]:#, args.ckpt_pathRL
                ...
                # encoder.load_state_dict(torch.load('/home/eric/mwnl/ckpt_AWGN_CE_Stage2/encoder_epoch201.pth', map_location='cpu'))
                # decoder.load_state_dict(torch.load('/home/eric/mwnl/ckpt_AWGN_CE_Stage2/decoder_epoch201.pth', map_location='cpu'))
                # embeds_shared.load_state_dict(torch.load('/home/eric/mwnl/ckpt_AWGN_CE_Stage2/embeds_shared_epoch201.pth',  map_location='cpu'))
                for _ in range(1):
                    ...
                    print('{}'.format(res))
                    print("Message decoded. Decoded bits: 1")
                    print('-----------------------------------------------')
                    print('------------------Comparison-------------------')
                    print('-----------------------------------------------')
    ...
    # print(f"Elapsed time for test: {elapsed_time:.2f} seconds")
